...
const AdminPricing: React.FC = () => {
  ...
  const setActive = (key: string, active: boolean) => {
    setPlans((prev) => ({ ...prev, [key]: { ...prev[key], active } }));
  };
                ...
                <CardContent className="p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch checked={cfg.active !== false} onCheckedChange={(v) => setActive(key, v)} />
                      <span className={`text-xs font-medium ${cfg.active !== false ? 'text-green-600' : 'text-muted-foreground'}`}>
