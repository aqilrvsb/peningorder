...
serve(async (req) => {
  ...
  try {
    ...
    const findRow = async (): Promise<any> => {
      const cols = "id, tracking_number, delivery_status, kurier, name_customer, phone_customer, marketer_id_staff, id_sale, pd_order_id, owner_user_id, address_customer, city_customer, postcode_customer, state_customer, total_sale, nota_staff, date_return, bundle:logistic_bundles(name)";
      if (consignNo) {
          ...
          if (isDelivered || isReturn) {
            await supabase
              ...
              .update({
                ...
                // Stamp date_return on a return so it surfaces in the Return tab
                // (which filters by date_return). Only set it once.
                ...(isReturn && !matched.date_return ? { date_return: mYmd } : {}),
